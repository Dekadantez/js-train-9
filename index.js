...
function findElementGreaterThan(arr, num) {
  ...
  const elem = arr.find((element) => element > num);
  return elem;
...
function findIndexByProperty(arr, name) {
  ...
  let fullName = "Petro";
  const index = arr.findIndex(({ name }) => name === fullName);
  return index;
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const subArray = arr.slice(startIdx, arr.length, arr);
...
function getArrayKeys(arr) {
  ...
  return Array.from(iterator, function (iterator) {
    return iterator;
  });
...
function getPositiveArrayValues(arr) {
  ...
  const filtred = (value, index, iter) => value > 0;
  const resultFiltr = iter.filter(filtred);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const res = n.map((r) => r ** 2);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  const free = (value, index, res) => value > threshold;
  const filteredSquaredArray = res.filter(free);
...
function wordLengths(arr) {
  ...
  const lengthsArray = arr.map((n) => n.length / [n].length);
...
function insertElementAt(arr, index, element) {
  ...
  const result = arr.splice(1, 2, index, element, 3);
...
function expandArrayByN(arr, n) {
  ...
  const result = arr.flatMap((elem) => (n = [elem, elem, elem]));
...
function findLongestWord(arr) {
  ...
  const result = arr.reduce((num, value) => {
    return value.length > num.length ? value : num;
  }, "");
  let longestWord = result;
  return longestWord;
...
function findDuplicateElements(arr) {
  ...
  const filFn = arr.filter((value, index) => arr.indexOf(value) !== index);
...
function calculateTotalPrice(arr) {
  ...
  const sum = arr.reduce((num, value) => num + value.price, 0);
  return sum;
...
function findLastNegativeNumberIndex(arr) {
  ...
  const result = (elem) => (elem = [elem].length);
  return arr.findLastIndex(result);
